...
const EMOTIONS = [
  { id: "calm", name: "平静", emoji: "🌿", tone: "steady" },
  { id: "happy", name: "开心", emoji: "☀️", tone: "up" },
  { id: "anxious", name: "焦虑", emoji: "🌊", tone: "down" },
  { id: "tired", name: "疲惫", emoji: "☁️", tone: "down" },
  { id: "sad", name: "低落", emoji: "🌧️", tone: "down" },
  { id: "angry", name: "烦躁", emoji: "🔥", tone: "down" },
  { id: "lonely", name: "孤单", emoji: "🌙", tone: "down" },
  { id: "hopeful", name: "期待", emoji: "✨", tone: "up" },
];
...
function renderEmotions() {
  ...
  grid.innerHTML = EMOTIONS.map(
    ...
    <button type="button" class="emotion-btn ${e.id === selectedEmotion ? "is-selected" : ""}" data-emotion="${e.id}" role="option" aria-selected="${e.id === selectedEmotion}">
      <span class="emotion-emoji" aria-hidden="true">${e.emoji}</span>
...
function saveEntry() {
  ...
  hint.hidden = false;
  hint.textContent = "已保存。可到「自助方案」看看此刻适合做什么。";
...
function renderInsight() {
  ...
  if (!entries.length) {
    stats.innerHTML = `
      <div class="stat"><strong>0</strong><span>条日记</span></div>
  ...
  stats.innerHTML = `
    <div class="stat"><strong>${entries.length}</strong><span>条日记</span></div>
    <div class="stat"><strong>${avg}</strong><span>平均强度</span></div>
    <div class="stat"><strong>${topEmotion.emoji} ${topEmotion.name}</strong><span>出现最多</span></div>`;
  ...
  bars.innerHTML = EMOTIONS.filter((e) => emotionCount[e.id])
    ...
    .map((e) => {
      ...
      return `<div class="bar-item">
        <div class="rank-top"><span>${e.emoji} ${e.name}</span><em>${count} 次</em></div>
...
function renderCare() {
  ...
  context.textContent = latest
    ? `基于最近一次记录：${emotion.emoji} ${emotion.name} · 强度 ${latest.intensity}/10。${pack.context}`
...
function renderHistory() {
  ...
  if (!entries.length) {
    timeline.innerHTML = `<div class="empty">还没有日记。去「今日记录」写第一条吧。</div>`;
    return;
  }
  timeline.innerHTML = entries
    .map((e) => {
      ...
      return `<article class="entry">
        <div class="entry-top">
          <div class="entry-mood">${emotion.emoji} ${emotion.name} · ${e.intensity}/10</div>
